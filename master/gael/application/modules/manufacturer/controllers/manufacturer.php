...
class Manufacturer extends MY_Sublimecontroller {
	...
	function _index($start = 0)
	{
            ...
            {
                $q = $this->input->post('query_manufacturer');
                var_dump($q);
                $this->session->set_flashdata('query_manufacturer', $q);
                redirect($this->input->server("REQUEST_URI"));
            }
            elseif($this->session->flashdata('query_manufacturer'))
            {
                $q = $this->session->flashdata('query_manufacturer');
                $this->session->set_flashdata('query_manufacturer', $q);
            }
            else
            {
