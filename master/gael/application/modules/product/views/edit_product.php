						...
						<fieldset>
								...
								<?php $error = ''; if(form_error('type')){ $error = 'error'; } ?>
							   	...
							   	<div class="control-group formSep <?php echo $error; ?>">
							        ...
							        <div class="controls">
									    <select name="type" id="type" class="input-xlarge chosen">
							                <option value=""><?php echo $this->lang->line('select_tax_class'); ?></option>
							                <?php if(isset($type_records)) : foreach($type_records as $key => $val) :
												$selected = "";
							                	if ( $this->input->post("type") )  {
													if( $this->input->post("type") == $val->type){ $selected = "selected"; }
												}else{
													if($product_records->type == $val->type ) { $selected = "selected"; }
												}
							                ?>
											<option value="<?php echo $val->type; ?>" <?php echo $selected; ?>  >
									    ...
									    <select name="manufacturer_id" id="manufacturer_id" class="input-xlarge chosen">
							                ...
							                <?php if(isset($manufacturer_records)) : foreach($manufacturer_records as $key => $val) :
							                	...
							                	if ( $this->input->post("manufacturer_id") )  {
													if( $this->input->post("manufacturer_id") == $val->manufacturer_id){ $selected = "selected"; }
												}else{
													if($product_records->manufacturer_id == $val->manufacturer_id ) { $selected = "selected"; }
												}
							                ?>
											<option value="<?php echo $val->manufacturer_id; ?>" <?php echo $selected; ?>  >
									    ...
									    <select name="tax_class_id" id="tax_class_id" class="input-xlarge chosen">
							                ...
							                <?php if(isset($tax_class_records)) : foreach($tax_class_records as $key => $val) :
							                	...
							                	if ( $this->input->post("tax_class_id") )  {
													if( $this->input->post("tax_class_id") == $val->tax_class_id){ $selected = "selected"; }
												}else{
													if($product_records->tax_class_id == $val->tax_class_id ) { $selected = "selected"; }
												}
							                ?>
											<option value="<?php echo $val->tax_class_id; ?>" <?php echo $selected; ?>  >
											<?php echo $val->ratio . ' ' . $val->name; ?></option>
											<?php endforeach; ?>
											<?php endif; ?>
							            </select>
										<?php echo form_error('tax_class_id', '<span class="help-inline">', '</span>'); ?>
